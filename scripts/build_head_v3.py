"""
Unum Avatar Head Builder v3.
Uses mediapipe landmarks to create a 3D head mesh with properly baked texture.
Instead of projecting the photo as-is, we rasterize each triangle and sample
the source image at the correct landmark pixel positions.
"""
...
def compute_uvs_packed(pts, max_v):
    """Compute UV layout that packs the face into [0,1] UV space.

    Map the face landmark bounding box to fill UV space,
    with margin for texture bleeding.
    """
    ...
    uvs[:, 1] = 1.0 - (margin + (1.0 - 2*margin) * (face_pts[:, 1] - y_min) / y_range)

    return uvs


def bake_texture(image_path, pts, uvs, faces, max_v, tex_size=2048):
    """Bake face texture using cv2.warpAffine per triangle for speed."""
    src = cv2.imread(image_path)
    src_h, src_w = src.shape[:2]
    # Pre-fill with average face color to hide seams
    ...
    # For original vertices, source pixel = landmark position in image
    # For subdivided vertices, interpolate via UV→source affine per triangle
    # We compute source coords for ALL vertices by projecting UVs back to image space
    # Since UVs were derived from pts[:max_v, :2], we can invert that mapping
    ...
    # Invert UV mapping: uv → normalized coords → image pixel
    src_px = (((uvs[:, 0] - margin) / (1.0 - 2*margin)) * x_range + x_min) * src_w
    src_py = (((1.0 - uvs[:, 1] - margin) / (1.0 - 2*margin)) * y_range + y_min) * src_h

    uv_px = uvs[:, 0] * tex_size
    uv_py = (1.0 - uvs[:, 1]) * tex_size

    for face in faces:
        i0, i1, i2 = face

        uv_tri = np.array([[uv_px[i0], uv_py[i0]],
                           [uv_px[i1], uv_py[i1]],
                           [uv_px[i2], uv_py[i2]]], dtype=np.float32)
        src_tri = np.array([[src_px[i0], src_py[i0]],
                            [src_px[i1], src_py[i1]],
                            [src_px[i2], src_py[i2]]], dtype=np.float32)

        # Bounding rect in texture space
        x_min = max(0, int(np.floor(uv_tri[:, 0].min())) - 1)
        y_min = max(0, int(np.floor(uv_tri[:, 1].min())) - 1)
        x_max = min(tex_size, int(np.ceil(uv_tri[:, 0].max())) + 1)
        y_max = min(tex_size, int(np.ceil(uv_tri[:, 1].max())) + 1)
        w, h = x_max - x_min, y_max - y_min
        if w < 1 or h < 1:
            continue

        # Offset triangles to local crop coords
        uv_local = uv_tri - np.array([x_min, y_min], dtype=np.float32)

        # Affine warp: source triangle → UV crop
        M = cv2.getAffineTransform(src_tri, uv_local)
        patch = cv2.warpAffine(src, M, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_REPLICATE)
        ...
        # Paste into texture
        roi = texture[y_min:y_max, x_min:x_max]
        np.copyto(roi, patch, where=(mask[:, :, None] > 0))

    out_path = os.path.join(OUTPUT_DIR, "unum_head_texture_v3.png")
    cv2.imwrite(out_path, texture)
    return out_path


def subdivide_mesh(verts, faces, uvs, n_iters=2):
    """Loop-style subdivision: split each triangle into 4 by inserting edge midpoints."""
...
def landmarks_to_3d(pts, scale=150.0):
    ...
    verts[:, 1] = -(pts[:, 1] - cy) * scale
    verts[:, 2] = -pts[:, 2] * scale * 0.6
    return verts
...
def pad_morph(delta, total_verts, original_verts):
    """Pad morph target deltas with zeros for subdivided vertices."""
...
def write_glb(filepath, verts, faces, uvs, normals, texture_path,
    ...
    gltf = {
        "asset": {"version": "2.0", "generator": "unum-avatar-v3"},
...
def main():
    ...
    print("=== Unum Avatar Head v3 (baked texture) ===\n")

    print("[1/6] Detecting landmarks...")
    pts, bs, image = detect_landmarks(INPUT_IMAGE)
    print(f"  {len(pts)} landmarks")

    print("[2/6] Building topology...")
    faces = get_face_mesh_triangles()
    max_v = 468
    faces = faces[np.all(faces < max_v, axis=1)]
    print(f"  {len(faces)} triangles")

    print("[3/7] Building 3D mesh...")
    verts = landmarks_to_3d(pts[:max_v])

    # Fix winding order: all face normals should point outward (positive Z)
    fixed_faces = []
    for tri in faces:
        i0, i1, i2 = tri
    ...
    print("[4/7] Computing packed UVs...")
    uvs = compute_uvs_packed(pts, max_v)

    print("[5/7] Subdividing mesh (2 iterations)...")
    ...
    print("[6/7] Baking face texture...")
    tex_path = bake_texture(INPUT_IMAGE, pts, uvs, faces, max_v, TEX_SIZE)
    print(f"  Texture: {tex_path}")

    print("[7/7] Generating blendshapes + export...")
    ...
    glb_path = os.path.join(OUTPUT_DIR, "unum_head_v3.glb")
    ...
    print(f"\n=== Done! Open viewer.html to preview ===")
